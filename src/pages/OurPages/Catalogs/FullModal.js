...
const FullscreenModal = ({ fullScreenModal = false, handleModalChange = () => { }, heading, singleProductState, handleApiCall, catagoryId }) => {
    ...
    const attributesRes = useSelector((state) => state.Catalog.ProductAttributeById);
    ...
    return (
        <Modal size="xl" isOpen={fullScreenModal} toggle={() => handleModalChange(false)} className="modal-fullscreen" id="exampleModalFullscreen">
            <ModalHeader toggle={() => handleModalChange(false)}>{heading}</ModalHeader>
            <ModalBody>
                <Row>
                    <Col lg={12}>
                        <FormGroup>
                        ...
                        <Col lg={4} key={index}>
                            <FormGroup>
                                ...
                                <Input
                                    type="select"
                                    onChange={(e) => {
                                        const updatedValues = [...attributesValues.filter(val => val.key !== attribute.name), { key: attribute.name, value: e.target.value }];
                                        setAttributesValues(updatedValues);
                                    }}
                                >
                                    <option value="">Select...</option>
                                    {attribute.values.map((value, idx) => (
                                        <option key={idx} value={value} selected={attributesValues.some(val => val.key === attribute.name && val.value === value)}>
                                            {value}
                                        </option>
                                    ))}
                                </Input>
            ...
            <div className="modal-footer">
                <Link to="#" onClick={() => handleModalChange(false)} className="btn btn-link link-success fw-medium">
